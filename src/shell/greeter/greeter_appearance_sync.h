...
namespace greeter {
  ...
  // `/var/lib/noctalia-greeter/appearance.json` on next login. The manifest includes
  // wallpaper/colors plus `session` (shell.session.actions and shell.session.power).
  // When multi-monitor layout is available from xdg-output, also stages `output_layout`
  // for greeter.toml.
